...
enum { NetSupported, NetWMName, NetWMState, NetWMStateAbove, NetWMCheck,
       ...
       NetClientList, NetClientListStacking, NetClientInfo, NetDesktopNames,
...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	drw_rect(drw, x - 2 * sp, barborder, w, bh, 1, 1);
	x += horizpadbar / 2;
	...
	while (text[++i]) {
		if (text[i] == '^' && !isCode) {
			...
			text[i] = '\0';
			w = TEXTW(text) - lrpad;
			drw_text(drw, x - 2 * sp, barborder, w, bh, 0, text, 0);
			...
			while (text[++i] != '^') {
				if (text[i] == 'c') {
					...
					int c = atoi(text + ++i);
					drw_clr_create(drw, &drw->scheme[ColBg], termcolor[c], alphas[SchemeNorm][ColFg]);
				} else if (text[i] == 'd') {
					drw->scheme[ColFg] = scheme[SchemeNorm][ColFg];
					drw->scheme[ColBg] = scheme[SchemeNorm][ColBg];
				} else if (text[i] == 'w') {
					...
					drw_rect(drw, rx + x - 2 * sp, ry + barborder, rw, rh, 1, 0);
				} else if (text[i] == 'f') {
					x += atoi(text + ++i);
				}
			}
...
setup(void)
{
	...
	lrpad = drw->fonts->h;
	bh = barheight ? barheight : drw->fonts->h + vertpadbar + barborder * 2;
	sp = sidepad;
